...
const OptionChain = ({ onOptionSelect, onDataUpdate }) => {
  ...
  const [tradingOpportunities, setTradingOpportunities] = useState({ callOpportunities: [], putOpportunities: [] });
  ...
  const fetchOptionChain = async () => {
    ...
    try {
      ...
      if (formatted.underlying && formatted.underlying.ltp) {
        ...
        const estimatedIV = estimateImpliedVolatility(formatted.options, spotPrice);
        ...
        const levels = calculateSupportResistance(spotPrice, formatted.options, estimatedIV);
        setSupportResistance(levels);
        
        // Get current expiry date in DD-MM-YYYY format
        const expiryDate = formatted.expiryDates.length > 0 ? formatted.expiryDates[0].label : '';
        ...
        const optionsWithVolatilityAnalysis = optionsWithTheoreticalPrices.map(option => {
        ...
        const opportunities = identifyTradingOpportunities(optionsWithVolatilityAnalysis);
  ...
  const handleOptionSelect = (row, optionType) => {
    ...
    const expiryDate = expiryDates.find(date => date.value === selectedExpiry)?.label || '';
  ...
  const formatPriceDifference = (difference) => {
    ...
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Typography component="span" color={color} fontWeight="medium" fontSize="0.8rem">
          {prefix}{formatNumber(difference, 2)}
        </Typography>
      </Box>
    );
  };
  ...
  return (
    <Box sx={{ p: 2 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Typography variant="body2">
                Implied Volatility (IV): <b>{(impliedVolatility * 100).toFixed(1)}%</b>
              </Typography>
              <Typography variant="body2">
                Historical Volatility (HV): <b>{(historicalVolatility * 100).toFixed(1)}%</b>
              </Typography>
            </Grid>
            <Grid item xs={12} md={6}>
              {volatilitySkew && (
                <Box>
                  <Typography variant="body2">
                    ...
                    <Chip
                      label={volatilitySkew.skewRatio > 1.1 ? "Options Overpriced" : volatilitySkew.skewRatio < 0.9 ? "Options Underpriced" : "Fair Value"}
                      size="small"
                      color={volatilitySkew.skewRatio > 1.1 ? "error" : volatilitySkew.skewRatio < 0.9 ? "success" : "default"}
                      sx={{ ml: 1 }}
                    />
                  </Typography>
                  <Typography variant="body2">
                    IV-HV Difference: <b>{(volatilitySkew.skewDifference * 100).toFixed(1)}%</b> 
      ...
      {(tradingOpportunities.callOpportunities.length > 0 || tradingOpportunities.putOpportunities.length > 0) && (
        <Box sx={{ mb: 3 }}>
