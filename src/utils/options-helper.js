...
export const calculateImpliedVolatility = (
  ...
  let vol = Math.abs(Math.log(spotPrice / strikePrice)) / Math.sqrt(timeToExpiry);
...
export const calculatePutCallRatio = (optionChainData) => {
  ...
  return parseFloat((optionChainData.putOi / optionChainData.callOi).toFixed(2));
};
...
export const calculateMaxPain = (options) => {
  ...
  options.forEach(option => {
    ...
    options.forEach(checkOption => {
      ...
      if (option.call?.oi) {
        const callPain = Math.max(0, strikePrice - checkStrike) * option.call.oi;
