...
export const calculateSupportResistance = (
  ...
  const topCallStrikes = callOI.slice(0, TOP_STRIKE_COUNT).map((item) => item.strike);
  const topPutStrikes = putOI.slice(0, TOP_STRIKE_COUNT).map((item) => item.strike);
  ...
  const support = [
    ...topPutStrikes.filter((strike) => strike < spotPrice),
    ...volBasedSupport,
  ]
  ...
  const resistance = [
    ...topCallStrikes.filter((strike) => strike > spotPrice),
    ...volBasedResistance,
  ]
 ...
 */
export const estimateImpliedVolatility = (
  optionsData,
  spotPrice,
  expiryInSeconds
) => {
...
const estimateIVFromPrice = (
  ...
  // TODO: rskFreeRate debt should be entered through input
 ...
 */
export const calculateTheoreticalPrices = (
  optionsData,
  spotPrice,
  expiryDate,
  ivEstimate = 0.3
  ...
  return optionsData.map((option) => {
    ...
    if (option.call) {
      ...
      enhancedOption.call = {
        ...
        percentDifference:
          ((option.call.ltp - theoreticalPrice) / theoreticalPrice) * 100,
    ...
    if (option.put) {
      ...
      enhancedOption.put = {
        ...
        percentDifference:
          ((option.put.ltp - theoreticalPrice) / theoreticalPrice) * 100,
        greeks: calculateGreeks(
          'put',
          spotPrice,
          option.strikePrice,
...
export const identifyTradingOpportunities = (optionsWithTheoreticalPrices) => {
  ...
  optionsWithTheoreticalPrices.forEach((option) => {
    if (
      option.call &&
      Math.abs(option.call.percentDifference) >= THRESHOLD_PERCENT
    ) {
    ...
    if (
      option.put &&
      Math.abs(option.put.percentDifference) >= THRESHOLD_PERCENT
    ) {
      const opportunity = {
        strike: option.strikePrice,
